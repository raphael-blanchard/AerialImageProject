...
void FireSimulator::nextStep()
{
    ...
    else
    {
        ...
        if ((int)potentialBurningPixels.size() > 0)
        {
            ...
            //normal distribution would've been there.
...
void FireSimulator::Simulation()
{
    string simulationInput;
    int simulationStep = 0;
    cout << endl;
    cout << "THIS IS THE START OF THE SIMULATION, PLEASE OPEN THE FILE CALLED simulationFile.svg AS SOON AS YOU PRESSED ENTER WITH AN SVG VIEWER TO OBSERVE HOW THE SIMULATION GOES ON." << endl;
    cout << "ENTER key to continue, ANY OTHER key to stop" << endl;
    getline(cin, simulationInput);
    while (simulationInput.size() == 0)
    {
        Image simulationImg = getImageOfStepN(simulationStep);
